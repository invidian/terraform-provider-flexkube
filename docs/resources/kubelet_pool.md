...
* `extra_mount` - (Optional) A `extra_mount` block as defined below. Can be specified zero or more times. Each block represents extra mount to be added to all kubelet containers managed by this pool.
...
A `extra_mount` block supports the following:
